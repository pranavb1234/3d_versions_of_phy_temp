...
2. Landing page appears with project title, background video, flow image, and simulation cards.
3. User selects a chapter: Oscillations, Waves, or Optics.
4. User chooses a simulation from that chapter.
5. The main lab interface opens.
6. User changes simulation parameters using buttons, sliders, or drag controls.
7. The visual scene updates immediately.
8. Formula panels and calculation values update.
9. User can open resource PDFs from the Resources panel.
10. User can switch to another simulation or return to the main page.
